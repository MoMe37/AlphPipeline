...
def compute_adjacent_by_edges(mesh: Mesh):
...
def compute_adjacent_by_vertices(mesh: Mesh):
...
def max_triangle_length(mesh: Mesh):
    a, b, c = mesh.span_components()
    return max(np.max(np.linalg.norm(a, axis=1)), np.max(np.linalg.norm(b, axis=1)))


def match_triangles(source: Mesh, target: Mesh, factor=2) -> List[Tuple[int, int]]:
...
def apply_markers(A: sparse.spmatrix, b: np.ndarray, target: Mesh, markers: np.ndarray) \
...
def revert_markers(A: sparse.spmatrix, x: np.ndarray, target: Mesh, markers: np.ndarray,
...
def compute_correspondence(source_org: Mesh, target_org: Mesh, markers: np.ndarray, plot=False) \
        -> np.ndarray:
    #########################################################
    # Configuration
    ...
    for iteration in range(iterations):
        ...
        result = Mesh(vertices=vertices[:len(source_org.vertices)],
...
def get_correspondence(source_org: Mesh, target_org: Mesh, markers: np.ndarray,
...
if __name__ == "__main__":
    cfg = ConfigFile.load(ConfigFile.Paths.highpoly.horse_camel)
    # Load meshes
    source_org = Mesh.load(cfg.source.reference)
    target_org = Mesh.load(cfg.target.reference)
