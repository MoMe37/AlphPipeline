...
def plot_example_alphsistant(ref_paths, result_paths): 

    # cat_path = "models/lowpoly/cat/cat_reference.obj"

    # cat = Mesh.load(cat_path)
    vis = BrowserVisualizer()
    for i, (ps, pt) in enumerate(zip(ref_paths, result_paths)): 
        ms = Mesh.load(ps)
        mt = Mesh.load(pt)
        vis.add_mesh(ms, offset = (ms.size()[0] * i,0,0))
        vis.add_mesh(mt, offset = (mt.size()[0] * i,-1.05 * mt.size()[1],0))
    vis.show()
...
if __name__ == "__main__":
    test_path = "C:/Users/Enzo.Magal/Documents/Enzo2021/VidTIMIT/results/sa2_obj/face_000.txt.obj"
    train_path = "C:/Users/Enzo.Magal/Documents/Enzo2021/VidTIMIT/results/source_obj/face_001.obj"
    refs = [test_path]
    results = [train_path]
    plot_example_alphsistant(refs, results)
    # plot_example1()
    # plot_example2()
    # plot_voxel_cat()
