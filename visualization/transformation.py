...
from visualization.correspondence import get_correspondence, compute_adjacent_by_edges, TransformMatrix
...
class Transformation:
    ...
    @classmethod
    def _compute_mapping_matrix(cls, target: Mesh, mapping: np.ndarray):
    ...
    @classmethod
    def _compute_missing_smoothness(cls, target: Mesh, mapping: np.ndarray):
    ...
    def __call__(self, pose: Mesh) -> Mesh:
        ...
        vertices = x
        result = Mesh(vertices=vertices[:len(self.target.vertices)], faces=self.target.faces)
        return result
...
if __name__ == "__main__":
    ...
    original_pose = Mesh.load(cfg.source.poses[0])
